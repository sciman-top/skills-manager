...
Describe 'MCP machine-readable planning' {
    ...
    It 'does not write managed targets or invoke native mutation in plan mode' {
        ...
        $beforeCfgHash = (Get-FileHash -LiteralPath $script:CfgPath -Algorithm SHA256).Hash
        ...
        (Get-FileHash -LiteralPath $script:CfgPath -Algorithm SHA256).Hash | Should Be $beforeCfgHash
        Assert-MockCalled Invoke-NativeMcpSync -Times 0 -Exactly
        Assert-MockCalled Invoke-NativeMcpCleanup -Times 0 -Exactly
    }

Describe 'vNext product planning contract' {
    ...
    function Invoke-PlanningVerifier([string]$Root, [string[]]$ExtraArgs = @()) {
        $output = @(& pwsh -NoProfile -ExecutionPolicy Bypass -File $scriptPath -RepoRoot $Root -Json @ExtraArgs 2>&1)

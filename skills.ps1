...
param(
    [ValidateSet("menu", "初始化", "新增技能库", "删除技能库", "发现", "安装", "卸载", "选择", "构建生效", "更新", "锁定", "打开配置", "解除关联", "清理备份", "帮助", "doctor", "add", "npx", "安装MCP", "卸载MCP", "同步MCP", "mcp-install", "mcp-uninstall", "mcp-sync")]
    [string]$Cmd = "menu",
    [string]$Filter = "",
    [switch]$DryRun,
    [switch]$Locked,
...
function Format-MappingPreview($items, [string]$targetPrefix = "") {
    ...
    return ($items | ForEach-Object { "$prefix$($_.vendor) :: $($_.from) -> $($_.to)" })
}
function Format-SkillPreview($items) {
    return ($items | ForEach-Object { "$($_.vendor) :: $($_.from)" })
}
function Preflight {
...
function Invoke-Git([string[]]$GitArgs) {
    ...
    if ($LASTEXITCODE -ne 0) { throw ("git 失败：git {0}" -f ($GitArgs -join " ")) }
}
...
function 安装 {
    ...
    { param($idx, $item)
        ...
        if ($item.from -eq ".") { $leaf = $item.vendor }
        return ("{0,3}) [{1}] {2}" -f $idx, $item.vendor, $leaf)
    } `
        "请选择要安装的技能（批量安装到白名单）" `
    ...
    foreach ($item in $selected) {
        ...
        $added++
        $previewMappings += [pscustomobject]@{ vendor = $item.vendor; from = $item.from; to = $to }
    }
...
function 卸载 {
    ...
    { param($idx, $item)
        $label = if ($item.vendor -eq "manual") { "manual" } else { $item.vendor }
        $leaf = Split-Path $item.from -Leaf
...
function 发现 {
    Invoke-WithMetric "discover" {
        ...
        Write-ItemsInColumns $list { param($idx, $item)
            ...
            $leaf = Split-Path $item.from -Leaf
            if ($item.from -eq ".") { $leaf = $item.vendor }
            return ("{0,3}) [{1}] [{2}] {3}" -f $idx, $mark, $item.vendor, $leaf)
        }
    } @{ command = "发现" } -NoHost
}
...
function 清空Agent目录 {
    if (Test-Path $AgentDir) {
        Invoke-RemoveItem $AgentDir -Recurse
    }
    EnsureDir $AgentDir
}
...
function Rollback-BuildTransaction($txn) {
    ...
    try {
        if (Test-Path $AgentDir) { Invoke-RemoveItem $AgentDir -Recurse }
        ...
        if (Test-Path $txn.path) { Invoke-RemoveItemWithRetry $txn.path -Recurse -IgnoreFailure | Out-Null }
    }
}
...
function 构建Agent($cfg = $null, [switch]$SkipPreflight) {
    return (Invoke-WithMetric "build_agent" {
        ...
        $nameToPaths = @{}
        foreach ($skillFile in (Get-ChildItem $AgentDir -Recurse -Filter "SKILL.md" -File -ErrorAction SilentlyContinue)) {
            $declaredName = $null
...
function 单技能安装 {
    ...
    Write-Host "提示：行尾用 \\ 可续行，脚本会自动拼接为一条命令。"
    $lines = New-Object System.Collections.Generic.List[string]
    while ($true) {
        $line = Read-Host "输入命令"
...
function Invoke-ParallelGitPrefetch($cfg, [int]$Parallelism = 1) {
    ...
    foreach ($p in $paths) {
        ...
        $job = Start-Job -ScriptBlock {
            param($repoPath)
            try {
                & git -C $repoPath fetch --all --tags | Out-Null
    ...
    return (Invoke-WithMetric "update_imports" {
        ...
        foreach ($i in $cfg.imports) {
            ...
            try {
                ...
                Ensure-Repo $cache $repo $ref $sparsePath $forceClean (-not $SkipFetch)
                $src = if ($skillPath -eq ".") { $cache } else { Join-Path $cache $skillPath }
                Need (Test-IsSkillDir $src) "未找到技能入口文件（SKILL.md/AGENTS.md/GEMINI.md/CLAUDE.md）：$src"
...
你需要的主要动作：
  1) 单技能安装：可粘贴多行命令（如：add ... 或 npx skills add ...）
  2) 新增技能库：输入 URL 自动写入并初始化（留空=仅初始化已有）
  3) 删除技能库：移除已配置 vendor 并重建生效
  4) 批量安装：从已添加的技能库选择并追加到白名单，自动构建生效
  5) 卸载：vendor 从白名单移除；manual 删除 imports 条目（兼容清理 legacy manual 目录）；overrides 备份后删除
  6) 手动更新：拉取上游并逐项确认本地改动处理 -> 重建 -> 同步
  7) 构建生效：仅用本地 imports(manual)/overrides/mappings 重建并同步（可配合 -Locked 严格锁定）
  8) 锁定：生成 skills.lock.json（记录 vendor/import 当前 commit）
  9) 解除关联：移除 link 关联（仅 link 模式需要）
 10) 清理备份：删除仓库内 *.bak.* 和 .bak 目录（排除 vendor/agent/imports/.git）
 11) 安装MCP：登记一个 MCP 服务到 skills.json（支持 stdio/sse/http），并自动同步
 12) 卸载MCP：从 skills.json 移除 MCP 服务，并自动同步
  注意事项：
  - 手动更新会拉取上游（vendor/imports），并逐项确认是否丢弃其本地改动。
  - 单技能安装默认严格预检：先校验仓库可达与技能路径，再执行安装。
  - 单技能安装会自动补全 owner/repo URL；若技能不唯一会提示候选路径。
  - 想只让本地改动生效，请用“构建生效”。
...
命令行：
  ...
  .\skills.ps1 发现（可选：单独查看可用 skills）
  通用参数：
...
本地技能：
  ...
  - 菜单 1 支持多行输入 add / npx skills add / npx add-skill。
...
function 菜单 {
    while ($true) {
        ...
        Write-Host "1) 单技能安装（粘贴 add / npx skills add ...）"
        Write-Host "2) 新增技能库（写入 vendors 并初始化）"
        Write-Host "3) 删除技能库（移除 vendor 并重建）"
        Write-Host "4) 批量安装（选择技能加入白名单）"
        Write-Host "5) 卸载（vendor 移除白名单；manual 删除 imports 条目；overrides 备份后删除）"
        Write-Host "6) 手动更新（拉取上游并逐项确认本地改动处理 -> 重建 -> 同步）"
        Write-Host "7) 构建并生效（仅用本地 imports(manual)/overrides/mappings 重建与同步）"
        Write-Host "8) 生成锁文件（skills.lock.json）"
        Write-Host "9) 解除关联（仅 link 模式需要）"
        Write-Host "10) 清理备份（删除仓库内 *.bak.* / .bak，排除 vendor/agent/imports/.git）"

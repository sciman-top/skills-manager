...
vNext P0-P5 已完成 repo-side 验收（P0/P1 各 9/9，P2/P3 各 7/7，P4/P5 各 6/6）。P5 将 selector 演进为 Adaptive Capability Fabric：schema v3 先理解 task type/domain/goal/operations，再组合最小 capability DAG、复用兼容 session capability、输出 `apply=false` profile preheat，并消费 Codex App Server 当前只读 skill/app/MCP snapshot。current snapshot 以字段级 truth merge 覆盖 availability/auth/callability/freshness，静态配置保留 path/policy/profile reachability；plugin/MCP 安装、OAuth、host/profile/session 写入、重启与 `live_accepted` 仍不在边界。
...
P5 schema v3 在兼容上述字段的基础上增加 `task_model`、`capability_graph`、`host_snapshot`、`session_plan`、`preheat_recommendation` 和统一 `inventory`。`scripts/get-codex-app-server-capability-snapshot.ps1` 只调用已由当前 manual/schema/实测证明的只读 App Server RPC；`app/read(includeTools=true)` 在当前 host 返回 403，因此工具级事实采用成功的 `mcpServerStatus/list` annotations 与 `codex_apps` namespace 映射。MCP 缺省 annotation 按 `readOnlyHint=false`、`destructiveHint=true`、`openWorldHint=true` fail-closed；显式非只读不会被名称启发式降级，复合请求按 read/write/destructive 类别选工具并聚合最高风险。`scripts/verify-capability-routing.ps1 -HostSnapshotPath <snapshot>` 对每个 snapshot capability 做 identity probe，并核对 annotation/tool gate，不只依赖手写 golden corpus。未向当前任务暴露的 MCP 保持 `request_mcp_activation`，不会把 server discovery 伪装成热加载。
...
`agent/` 已物化时，`skill-integrity`/`skill-routing` 严格检查 package/resource/installed closure；portable、CI 或 linked worktree 没有生成层时，gate 输出 `materialization_status=source_only`，只验证 tracked config/override/policy 与 profile closure，并列出 inactive/policy-only members。source-only 不等于 package 或 host runtime 已验证；恢复条件是运行 `构建生效` 后重跑 verifier，或用 fresh host snapshot/native probe 补足实时证据。

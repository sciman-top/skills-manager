...
- 当前 Lean Delivery maintenance：M0/M0.2 8/8 `repo_verified`；M0.2 只增加 host-owned coordination、single-writer/Git freshness、tool admission 和 verifier 合同，不实现 coordinator/lease runtime 或安装社区工具。M1 由 `tasks/skills-manager-vnext-lean-delivery-pilot.json` 保持 `collecting (0/10)`；不证明 pilot 已执行/完成、业务净收益、runtime/host mutation 或 live acceptance。

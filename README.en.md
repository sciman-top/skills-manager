...
vNext P0-P5 have passed repository-side acceptance (P0/P1: 9/9 each; P2/P3: 7/7 each; P4: 6/6; P5: 5/5). That historical status proves repository contracts, not natural-language routing quality. A P5-local maintenance correction now makes the host AI the semantic decision owner, keeps profiles as bounded preheat packages, and limits the compatibility router to profile-scoped discovery plus deterministic availability, containment, side-effect, and activation policy. Plugin/MCP installation, OAuth, silent host/profile/session writes, restart, and live acceptance remain out of scope.

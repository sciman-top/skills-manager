...
There are three delivery channels and four forms: public `general` Release installers (`bootstrap.zip` and `portable.zip`), the public Git source repository, and a private `private-all` migration snapshot. `bootstrap.zip` is the default installer and downloads locked sources through Git; `portable.zip` includes the prebuilt `agent/` directory for portable or network-constrained use. Both include an MIT `src/` source snapshot for inspection and rebuilding, but neither is a full development checkout: they contain no `.git` history or test suite. Use a Git clone or fork for ongoing development, branches, tests, contributions, and Git-based updates. `private-all` is a restore-oriented snapshot with per-file SHA-256 content evidence; by default it is a plaintext, private-use snapshot with no passphrase, while `--encrypt` creates an AES-256-GCM credential companion file. It is not a second development truth and does not include Git history. Use `git bundle --all` separately when offline history continuity is required. `rescan` is only a migration manifest mode, not a development distribution.

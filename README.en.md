...
The host AI continues to own long-chain task decomposition, subagent orchestration, and model/effort selection. The repository now implements runtime-independent TaskGraph, Radar snapshot, three soft model tiers, serial/parallel admission, and failure-escalation advice. `Sol xhigh / Sol medium / Luna max` are overrideable anchors, with Luna max as the current user default; this is not a runtime router. The repository does not spawn subagents, call a provider, fetch Radar, or mutate the active session, custom agents, provider/auth, or host config; host configuration and scheduled Radar refresh remain separate host-acceptance surfaces, while earlier Terra-high runs remain historical evidence only. Repository-side validation and planning commands are:
...
Output is fixed to `decision_owner=host_ai`, `executor=host_native_runtime`, and `provider_calls/native_mutations/writes=0`. The host uses native subagent/worktree surfaces after admission; shared files, generated seams, schema/migrations, Git/external state, final integration, and the full gate stay serial. The maximum current truth is `repo_verified/repo_advisory_only`, not `host_loaded/live_accepted`.

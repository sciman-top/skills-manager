...
The host AI continues to own long-chain task decomposition, subagent orchestration, and model/effort selection. The repository now implements runtime-independent TaskGraph, Radar snapshot, four soft model tiers, serial/parallel admission, and failure-escalation advice. `Sol xhigh / Sol medium / Terra high / Luna max` are overrideable defaults, with Terra high as the current user-preferred balanced tier; this is not a runtime router. The repository does not spawn subagents, call a provider, fetch Radar, or mutate the active session, custom agents, provider/auth, or host config; a separate host evaluation has observed native subagents and scheduled Radar refresh with partial-evaluation boundaries. Repository-side validation and planning commands are:

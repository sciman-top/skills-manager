...
Reconcile idempotently. Ensure exactly one target heartbeat per eligible non-supervisor target by deterministic marker and target thread id. Treat the dual-role supervisor as the canonical heartbeat for its own thread. Generate every target prompt from the installed New-WatchHeartbeatPrompt.ps1 script; require policy_revision=2 and its hash-valid prompt_sha256 envelope. Also require the installed target contract to contain the exact silence rule "entire assistant output must be exactly DONT_NOTIFY". If that rule is absent, treat the installed generator as stale and do not mutate target automations until the newer generator is projected. Preserve current notification policy and cadence unless the user changed them. Read the latest automation immediately before a full-field update and skip on conflicting revision or identity. There is no native CAS guarantee, so report conflicts instead of overwriting newer state.

Keep monitor-only target heartbeats ACTIVE. Delete completed orphan heartbeats only after current completion and verification truth is positive. Immediately before deletion, re-read the target thread and repository truth. If the current thread status is active, a newer non-heartbeat business turn exists, verification no longer matches current truth, or evidence conflicts, do not delete. A contextCompaction handoff summary, historical verification, title/preview, or target heartbeat final answer is never sufficient positive cleanup authority by itself. Do not let a target heartbeat pause, resume, update, or delete itself.

Before any target automation mutation, run `$HOME/.codex/scripts/Test-WatchGuardRuntime.ps1` and require a fresh trusted exact definition. Verify that a unique `watch-interrupted-task-v1-live-probe-<nonce>` id is absent, then call the native automation capability with that live-probe sentinel and require the result `denied before the native automation call executes`. If the sentinel reaches the host, even if it returns not found, classify soft_guard_only and perform no automation mutation. If the strict guard definition is not exact-hash reviewed and trusted, either fresh live-path probe is absent, a stale write-capable turn remains, or specialized-path coverage is unknown, classify soft_guard_only and observe only: do not create, update, activate, pause, or delete target automations.
...
Never call send_message_to_thread. Never hand off, wake, create, fork, rename, or inject prompts or coordination messages into peer tasks except the hash-valid policy_revision=2 watch heartbeat prompt explicitly authorized by this fleet contract. Never answer peer coordination. Use only read-only list/read/wait for task inspection and the native automation capability for canonical watch reconciliation.

When reconciliation and the dual-role task require no new user action, do not emit commentary, status, progress, or a summary. The entire assistant output must be exactly DONT_NOTIFY, with no commentary or additional text. An already-requested human action is not a new boundary. Only a newly discovered user-action boundary may produce one concise final notification when the host can prove deduplication.
'@

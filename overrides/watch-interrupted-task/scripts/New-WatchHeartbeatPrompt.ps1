...
Before recovery, require the user-level PreToolUse guard definition to be reviewed and trusted for its exact current hash and require a fresh-session live-path probe. Because specialized tool paths may opt out of the default hook path, treat the hook as a defense-in-depth guardrail. If trust, fresh-session loading, or live-path coverage is unproved, classify soft_guard_only, observe only, and keep the heartbeat ACTIVE. A currently running turn that predates the policy is stale_policy_running and is observe_only until it ends.

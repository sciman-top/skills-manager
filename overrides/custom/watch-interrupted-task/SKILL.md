...
- Ordinary fleet mode cannot mutate its own dual-role automation. The shutdown-armed mode may delete the supervisor only after every target heartbeat is gone, the stopped snapshot is stable on two ticks, the second tick has persisted a short-lived schedule authorization, and a final complete-visibility re-read consumes that exact authorization.
- Shutdown mode requires a non-empty monitored set, fresh `task_stopped=true` receipts for every target, `recovery_pending=false`, explicit RFC3339 timestamps, and the same deterministic snapshot on two consecutive ticks. A first tick plus `FinalRecheck` is never sufficient. Any stable stop reason is accepted. `unknown`, `soft_guard_only`, an active turn, any recovery/retry candidate, unsafe external-effect truth, or target-set change cancels the tick.

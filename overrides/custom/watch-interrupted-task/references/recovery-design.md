...
Persist the enrolled active membership in the ignored fleet journal together with generation, target automation/source-turn identity, last stop/notification/cleanup receipts, consecutive snapshot state, and successful shutdown receipts. The supervisor uses a five-minute cadence; target cadence remains ten minutes. Reaching 50 because of inactive history is not itself a blocker. Visible active unmonitored tasks, unavailable hosts/sources, identity/digest conflicts, recovery/retry, unsafe external effects, and unknown state still block. A newly visible active task joins membership and resets stability.

Once no target heartbeat remains, read every enrolled member again. Use `Get-WatchFleetShutdownDisposition.ps1` only with the current generation, supervisor automation id, native scheduled tick id, `UnmonitoredActiveTaskCount=0`, unavailable counts zero, and zero remaining target heartbeats. Each stopped record binds its target automation id, source turn id, evidence timestamp, external-effect state, checkpoint, and receipt. The exact same non-empty snapshot must be observed on two consecutive distinct scheduled ticks. Immediately re-list and re-read membership and automations before execution; a new active task or any drift cancels the tick. Re-run the helper and require an unchanged snapshot/shutdown receipt plus an unexpired short-lived execution window, then delete the supervisor heartbeat and verify its native receipt within that window. Only then schedule `shutdown.exe /s /t 120 /c "watch-interrupted-task: all monitored tasks stopped"`, never `/f`, and surface `shutdown /a` in the unique notification. Exit code 0 is the scheduling receipt. Reuse the stable receipt forever for the same snapshot; expiry limits only the first scheduling attempt, and failure or unknown effect stops without blind replay.

...
Before recovery, establish a fresh evidence timestamp, stable checkpoint_id, deterministic receipt_key, current Goal status, and external_effect_state. Never infer these from inactivity, a dirty worktree, TODO text, or an incomplete Goal alone. Pass the fresh tick timestamp as NowUtc, any stored retry boundary as NextRetryAtUtc, and the reviewed freshness window as EvidenceFreshnessMinutes to the bundled Get-WatchHeartbeatDisposition.ps1. Do not resume before next_retry_at. Proceed only when it returns resume_from_checkpoint, diagnose_replan_and_continue, reconcile_goal_and_continue, verify_goal_acceptance, or stop_after_verification.

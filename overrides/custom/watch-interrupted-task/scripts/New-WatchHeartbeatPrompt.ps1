...
At the start of every tick, read recent structured turns and pass them to Get-WatchHeartbeatDisposition.ps1 -TurnsJson. The current heartbeat turn itself never counts as an active business turn. Ignore every heartbeat envelope and classify running only when the latest non-heartbeat business turn was independently in progress and was not completed before this heartbeat envelope arrived. The task or thread status being active during the current scheduled tick is not running evidence. Inspect current repository or external-effect truth and the installed Goal state with get_goal when available. An active Goal alone is not completion evidence. A completed non-heartbeat turn with a recoverable 408/429/502/503/504 or transport error is recovery_pending=true, not running and not stopped.

Treat a target as naturally stopped only from positive business-turn evidence: the latest non-heartbeat business turn has a completed final answer and there is no active Goal or unfinished authorized checkpoint, or an explicit user pause/stop is proved. Then classify complete, natural_pause, or stopped as appropriate and obtain automation_action=request_supervisor_cleanup from the helper. In shutdown-managed mode the proved terminal disposition must also return quiesce_action=pause_self. Never infer a natural stop from inactivity alone.

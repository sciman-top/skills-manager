...
Before any recovery or stop decision, establish a fresh evidence timestamp, stable checkpoint_id, deterministic receipt_key, current Goal status, external_effect_state, no_active_turn, task_stopped, stop_reason, recovery_pending, and next_retry_at. Encode checkpoint_id as watch-checkpoint:<64 lowercase hex> and receipt_key as watch-receipt:<64 lowercase hex>, using SHA-256 over the canonical checkpoint or state-boundary evidence respectively; placeholders and free text are invalid. Never infer these from inactivity, a dirty worktree, TODO text, or an incomplete Goal alone. Run the bundled Get-WatchHeartbeatDisposition.ps1 with this structured state. Proceed with business recovery only when it returns resume_from_checkpoint, diagnose_replan_and_continue, reconcile_goal_and_continue, verify_goal_acceptance, or stop_after_verification.

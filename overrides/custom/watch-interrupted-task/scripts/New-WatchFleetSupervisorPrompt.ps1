...
Desktop permits at most one heartbeat attached to a task. Never create a separate target heartbeat for the supervisor thread. Apply the target recovery contract only to a latest non-heartbeat business objective that existed independently before fleet arming. The current fleet heartbeat turn itself is control-plane activity, never business activity; the supervisor control task is excluded when it had no independent business objective, and do not classify it running merely because this scheduled turn is in progress. In ordinary fleet mode this scheduled turn cannot mutate its own automation; only the separately armed shutdown extension below carries the direct user's exact final self-delete authority. A direct user lifecycle command otherwise owns supervisor pause, resume, update, and deletion.
...
Do not emit commentary or progress prose during a fleet heartbeat tick. DONT_NOTIFY suppresses notification only; Desktop may retain the native scheduled turn in task history, so keep routine receipts minimal. Finish with exactly this native XML shape and no text outside it. Copy automation_id from the heartbeat input. Routine reconciliation uses DONT_NOTIFY. Use NOTIFY only for a newly discovered, deduplicated human-action boundary.
...
For any control-plane fault, persist the first fault receipt before notifying. Bind the receipt to the normalized fault boundary, automation identity, generation, and affected target ids. The same control-plane fault receipt is DONT_NOTIFY on later ticks; never repeat the same fault text. A changed fault boundary may notify once with its new receipt. Missing or corrupt journal state must be repaired from fresh canonical automation and target XML evidence when safe; otherwise stay active and quiet after the first notification.
'@
}

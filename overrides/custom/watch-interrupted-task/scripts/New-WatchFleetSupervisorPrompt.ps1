...
After normal fleet reconciliation and stopped-target retirement, re-list automation metadata and require that no canonical target heartbeat remains. Re-read the broadest visible task set and fail closed when visibility is truncated or the host limit of 50 is reached without explicit proof that the set is complete. Include a newly active eligible task by enrolling its heartbeat and canceling shutdown. Build a non-empty snapshot covering the supervisor business task and every target retired under this fleet lifecycle that remains visible. Goal presence does not change this rule: each target AI decides whether its own business task stopped. Do not use a finite allowlist of stop causes. Accept any stable stop_reason only when fresh target evidence proves task_stopped=true, recovery_pending=false, no active business turn, no next_retry_at, a stable checkpoint_id and receipt_key, and safe or absent external-effect state. Known recovery or uncertain states override contradictory fields: running, resume_eligible, continuation_gap, recoverable_task_failure, strategy_drift, verification_failed, peer_busy, stale_policy_running, unknown, soft_guard_only, an unverified goal_satisfied state, or any 408/429/502/503/504 or transport-recovery boundary blocks shutdown.

Serialize one record per snapshot task with target_thread_id, automation_id, source_turn_id, state, task_stopped, stop_reason, recovery_pending, receipt_key, checkpoint_id, evidence_timestamp_utc, external_effect_state, next_retry_at, and no_active_turn. Require receipt_key in watch-receipt:<64 lowercase hex> form and checkpoint_id in watch-checkpoint:<64 lowercase hex> form; reject placeholders or caller-invented free text. Run the bundled Get-WatchFleetShutdownDisposition.ps1 with the current native heartbeat automation_id as AutomationId, the current heartbeat current_time_iso as CurrentTickId, ShutdownArmed, VisibilityComplete, UnmonitoredActiveTaskCount=0, and RemainingTargetHeartbeatCount=0. Use the preceding scheduled fleet XML current_tick_id and snapshot_key as PreviousTickId and PreviousSnapshotKey; use any prior successful shutdown receipt as PriorShutdownReceiptKey. The helper uses its own UTC clock for freshness and must return the same non-empty snapshot_key on two consecutive scheduled ticks whose CurrentTickId and PreviousTickId are distinct, ordered, and fresh before power_action=schedule_shutdown is eligible. Immediately before acting, re-list and re-read every visible task and automation; any changed set, newly active unmonitored task, target heartbeat, active turn, retry, stale evidence, unknown state, or mismatch in automation_id/source_turn_id cancels this tick. If the final re-read discovers a new active eligible task, enroll its canonical heartbeat under the same target policy and start stability observation again from a new snapshot.

For the one proved schedule_shutdown boundary, require a non-empty shutdown_receipt_expires_at_utc from the helper. Immediately before acting, repeat the complete visibility and snapshot re-read, re-run the helper with the same current scheduled tick identity, and require an unchanged snapshot_key and shutdown_receipt_key plus a newly returned, still-unexpired shutdown_receipt_expires_at_utc. Then delete this supervisor heartbeat with the current heartbeat automation_id and verify the native supervisor delete receipt before the latest expiry. If deletion is missing, conflicting, unproved, or the receipt expires, do not schedule shutdown. Only after the native supervisor delete receipt proves no watch heartbeat remains, and while the helper-issued receipt is still unexpired, execute exactly: shutdown.exe /s /t 120 /c "watch-interrupted-task: all monitored tasks stopped". Never add /f, /r, /p, /h, or a shell chain. Proceed only when the trusted shutdown-armed fleet prompt and cross-thread guard definition are active. Exit code 0 is the external-effect receipt. Emit NOTIFY with state=shutdown_scheduled, snapshot_key, shutdown_receipt_key, shutdown_receipt_expires_at_utc, supervisor_delete_receipt, and the cancellation command shutdown /a. Reuse the stable shutdown_receipt_key forever for the same snapshot and never replay the shutdown command; expiry limits only the first authorized scheduling attempt. If supervisor deletion or the shutdown command fails or its effect is unknown, emit one deduplicated NOTIFY and do not retry blindly.
'@
}

...
<message>state=...;receipt_key=...;tick_id=...;snapshot_key=...;shutdown_receipt_key=...;visibility_complete=true|false;list_limit_reached=true|false;visible_count=...;eligible_count=...;monitored_count=...;blocking_unmonitored_count=...;changed_count=...</message>
</heartbeat>
'@
...
After normal fleet reconciliation, build a non-empty snapshot covering the supervisor business task and every currently visible eligible local task. Goal presence does not change this rule: each target AI decides whether its own business task stopped. Do not use a finite allowlist of stop causes. Accept any stable stop_reason only when fresh target evidence proves task_stopped=true, recovery_pending=false, no active business turn, no next_retry_at, a stable checkpoint_id and receipt_key, and safe or absent external-effect state. Known recovery or uncertain states override contradictory fields: running, resume_eligible, continuation_gap, recoverable_task_failure, strategy_drift, verification_failed, peer_busy, stale_policy_running, unknown, soft_guard_only, an unverified goal_satisfied state, or any 408/429/502/503/504 or transport-recovery boundary blocks shutdown. Any visible/eligible task that is not represented by a fresh canonical target record, any enrollment conflict, and any unknown count is a blocking_unmonitored_count; never silently omit it.

Serialize one record per eligible target with target_thread_id, state, task_stopped, stop_reason, recovery_pending, receipt_key, checkpoint_id, evidence_timestamp_utc, external_effect_state, next_retry_at, and no_active_turn. Store the supervisor state journal under the current trusted repo's ignored reports/watch-interrupted-task/fleet directory, keyed by the supervisor automation id; pass that exact directory as StateRoot and the contained JSON leaf as StatePath. The helper rejects paths outside StateRoot and reparse-point state paths. If no safe writable repo state root exists, classify platform_na and do not arm shutdown. Run the bundled Get-WatchFleetShutdownDisposition.ps1 with ShutdownArmed, GuardReady, StateRoot, StatePath, TickId, VisibilityComplete, ListLimitReached, VisibleCount, EligibleCount, MonitoredCount, BlockingUnmonitoredCount, ConflictCount, and UnknownCount. The helper atomically records current_tick_id, previous_tick_id, snapshot_key, observed_at, and successful_shutdown_receipt_keys. A repeated TickId is rejected even when the caller repeats the same arguments. The helper must return the same non-empty snapshot_key on two consecutive scheduled ticks with distinct TickId values before power_action=schedule_shutdown is eligible. Immediately before acting, re-list and re-read every eligible task; any changed set, active turn, retry, stale evidence, or unknown state cancels this tick.

For the one proved schedule_shutdown boundary, execute exactly: shutdown.exe /s /t 120 /c "watch-interrupted-task: all eligible tasks stopped". Never add /f, /r, /p, /h, or a shell chain. Proceed only when the trusted shutdown-armed fleet prompt and cross-thread guard definition are active. Exit code 0 is the external-effect receipt; after exit code 0 call the helper once with the same TickId and ConfirmedShutdownReceiptKey so the durable successful_shutdown_receipt_keys set is updated. Emit NOTIFY with state=shutdown_scheduled, snapshot_key, shutdown_receipt_key, and the cancellation command shutdown /a. Reuse the receipt forever for the same snapshot and never replay the shutdown command; A→B→A is still blocked by the durable receipt set. If the command fails or its effect is unknown, emit one deduplicated NOTIFY and do not retry blindly.
'@
}

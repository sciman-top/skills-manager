...
| `agent_workflow_advisory_runtime` | Runtime-independent Agent workflow advisory | `repo_verified` + host evaluation partial | AWA-001..005 5/5；TaskGraph/FailurePacket v1、RadarSnapshot v2、completion receipt、barrier wave/admission、三档 host proposal validation 与 PS7 zero-write CLI 已实现；Luna max 已在 read-only CLI/provider surface 实跑，但 collaboration spawn 仍不可用；历史 v1 Radar receipt 不满足 v2 revalidation |

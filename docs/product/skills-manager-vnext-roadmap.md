...
| `agent_workflow_advisory_runtime` | Runtime-independent Agent workflow advisory | `repo_verified` + host evaluation partial | AWA-001..005 5/5；TaskGraph/FailurePacket v1、completion + execution receipt、2/4/1 委派预算、三档 Sol proposal/实际 agent pair 绑定与 PS7 zero-write CLI 已实现；custom-agent 后代委派已禁用，low/medium 生命周期因宿主缺 `close_agent` 且 open-thread slot 未释放而阻塞，xhigh 依用户要求暂缓；Radar 仅历史只读兼容 |

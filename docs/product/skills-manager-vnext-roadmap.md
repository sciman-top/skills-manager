...
| `agent_workflow_advisory_runtime` | Runtime-independent Agent workflow advisory | `repo_verified` + host evaluation partial | AWA-001..005 5/5；TaskGraph/Radar/FailurePacket、确定性 wave/admission、四档 soft anchor、FailurePacket escalation 与 PS7 zero-write CLI 已实现；Sol xhigh/Sol medium/Terra high 原生子代理、fresh host load、手工与首个 scheduled Radar snapshot 已有独立 host receipt，未来运行可靠性、Luna 可用性与业务 live 仍未证明 |
...
| `M0.3` | `repo_verified` | 落盘 host-owned TaskGraph/model policy、Radar snapshot、四档软锚点、failure escalation，以及 C#/.NET typed core + PowerShell thin shell 的条件性目标架构 | 3 个增量 planning tasks done；模型策略/typed-core/P6 边界负向 verifier 通过；唯一 full gate 通过；独立 host receipt 记录默认 Terra high、原生子代理和首个 Radar automation run | M0.3 只表示仓库 planning truth，不包含 host acceptance；本身未实现动态模型路由、Radar provider fetch、typed core/PoC 或 PowerShell 替换；当前 TC0/TC1 见独立 track；未来 scheduled reliability 与业务 live acceptance 未执行 |
...
M0.3 的模型策略由宿主 AI 执行：用户拥有目标、价值排序和不可逆授权；宿主生成 TaskGraph、决定串并行、选择/升级模型并综合结果；skills-manager 提供 Radar/成本/风险建议与 deterministic admission；Codex native runtime 实际 spawn/wait/steer/integrate。`Sol xhigh / Sol medium / Terra high / Luna max` 只是初始软锚点，Terra high 是当前用户覆盖的 balanced 默认；Radar 以显式、带过期时间的 snapshot 更新，实际同类任务返工/门禁/费用/时长优先于排行榜。缺工具/权限/凭据不允许通过升档伪装解决。
...
| `AWA-003` | `repo_verified` | `src/Application/ModelAndAgentPolicy.ps1`：deterministic waves、parallel admission、four soft tiers、user override and escalation | `discover -> implement+document -> integrate`；shared seam 串行；stale Radar 回退 | auto model switch |
...
波次与 admission 的执行顺序是：宿主生成 Product Baseline 与 TaskGraph → `agent-validate` → `agent-plan` → 宿主在通过的 wave 中使用 native spawn/worktree → 等待并收集结果 → integration owner 串行消费 candidate → affected/full gate。共享文件、生成链、schema/migration、lock/config、Git index/ref、同一外部状态和最终 closeout 永远串行；只有固定 base、依赖完成、精确 write set 互斥、独立验证和 owner 齐全才可并行。模型选择按 `user override -> local comparable outcomes -> host availability -> fresh Radar -> host default`，四档仅软锚点；FailurePacket 先根因后一次 corrected retry，能力不足才按 `Luna max -> Terra high -> Sol medium -> Sol xhigh` 有界升档，权限/凭据/授权/用户决策 fail-closed。

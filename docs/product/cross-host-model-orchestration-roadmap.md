...
| Claude Code | candidate，启用前一律 manual：Flash/high（轻量）；Flash/max constrained（有界）；V4 Pro/max（深度）；Pro/max + policy（高风险） | offline plan + contract tests | Claude 成功代表 ZCode/Codex，或 Pro/max 自动获批 |

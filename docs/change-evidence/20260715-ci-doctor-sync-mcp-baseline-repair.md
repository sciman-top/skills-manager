...
- write-set：`.github/workflows/ci.yml`、`README.md`、`README.en.md` 与本文件。
...
- 兼容性：`check-doctor-json.ps1` 的参数和 JSON 契约不变；本地或专用性能 lane 仍可不带 `-WarnOnly` 严格执行。
...
- hotspot/full：`pwsh -NoProfile -ExecutionPolicy Bypass -File scripts/quality/run-local-quality-gates.ps1 -Profile full -AllowDirtyWorktree`，exit `0`；关键输出为 `Local quality gates passed (full).`。

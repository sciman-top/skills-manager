...
- Final authoritative full gate on the stable executable snapshot: `pwsh -NoProfile -ExecutionPolicy Bypass -File scripts/quality/run-local-quality-gates.ps1 -Profile full -AllowDirtyWorktree` exited 0 in `434767ms`. Pester 4.10.1 passed Unit 951/951 across 76 files and E2E 18/18 across 2 files; the test suite elapsed `424626ms`.
...
1. exact diff and staged secret review;
2. `git diff --check` and exact staged review after this receipt-only insertion;
3. commit, push and remote-CI observation.

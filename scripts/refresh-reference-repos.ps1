...
foreach ($repoName in $RepoNames) {
    ...
    $redactedActualOrigins = @($actualOriginUrls | ForEach-Object { Get-RedactedGitRemote -Remote $_ })
    $redactedActualOrigin = $redactedActualOrigins -join ', '
    ...
    $originMatchesManifest = $originIdentityMatches
    ...
    if (-not $originIdentityMatches) {
        ...
        $results.Add([pscustomobject]@{
                ...
                actual_origin = $redactedActualOrigins
                ...
                note = ('origin identity verification blocked network operations; expected_upstream={0}; actual_origin={1}' -f $upstreamUrl, ($redactedActualOrigins -join ', '))
                compare_log = @()
            })
        continue
    }
